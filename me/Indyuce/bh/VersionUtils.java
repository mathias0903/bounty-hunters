...
public class VersionUtils implements Listener {
	...
	public static ItemStack[] getStatEquipment(Player p) {
		if (version.startsWith("v1_8")) {
			return new ItemStack[] { p.getInventory().getItemInHand(), p.getInventory().getHelmet(),
		...
		return new ItemStack[] { p.getInventory().getItemInMainHand(), p.getInventory().getItemInOffHand(),
				p.getInventory().getHelmet(), p.getInventory().getChestplate(), p.getInventory().getLeggings(),
	...
	public static void sound(Location loc, String sound, float vol, float pitch) {
		...
		try {
			...
			Bukkit.getConsoleSender().sendMessage("§4[Bounty Hunters] Bug with " + sound
					+ ". No such sound found. Please report it to the plugin creator :)");
		}
	}
